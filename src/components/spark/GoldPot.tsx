...
const GoldPot = ({
  ...
  useEffect(() => {
    if (showRewardAnimation) {
      ...
      const timeline = [{
        delay: 200,
        action: () => setShowCoins(true)
      }, {
        delay: 600,
        action: () => setShowBounce(true)
      }, {
        delay: 800,
        action: () => setLocalFill(prev => Math.min(prev + 10, 100))
      }, {
        delay: 1000,
        action: () => setShowSparkles(true)
      }, {
        delay: 1400,
        action: () => setShowBounce(false)
      }, {
        delay: 2000,
        action: () => {
          setShowCoins(false);
  ...
  return <div className={`relative ${config.container} flex items-center justify-center`}>
      {/* Ambient glow - premium gold tones */}
      <div className="absolute inset-0 animate-pulse-glow" style={{
      opacity: 0.4 + localFill / 100 * 0.4
    }}>
        <div className="absolute inset-4 rounded-full blur-2xl transition-all duration-500" style={{
      ...
      {/* Sparkle particles */}
      {showSparkles && <>
          {[...Array(12)].map((_, i) => <div key={i} className="absolute animate-sparkle z-20" style={{
      ...
      {showCoins && <>
          {[...Array(8)].map((_, i) => <div key={i} className="absolute animate-coin-drop z-10" style={{
        left: `${20 + i * 8}%`,
        top: "-15%",
      ...
      <div className={`relative ${showBounce ? "animate-shake" : ""}`}>
        ...
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 rounded-full blur-xl transition-all duration-700" style={{
        ...
        {showCoins && localFill > 30 && <div className="absolute top-2 left-1/2 -translate-x-1/2 flex gap-1 z-20">
            {[...Array(3)].map((_, i) => <span key={i} className="text-xl animate-bounce" style={{
      ...
      {/* Reward amount popup */}
      {showRewardAnimation && rewardAmount > 0 && showSparkles && <div className="absolute -top-4 left-1/2 -translate-x-1/2 animate-slide-up z-30">
